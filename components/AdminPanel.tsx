...
import { getOrders, updateOrderStatus, getProducts, saveProduct, deleteProduct, PRODUCT_CATEGORIES, getStoreConfig, saveStoreConfig } from '../services/dbService';
import { 
  CheckCircle, MapPin, Phone, 
  RefreshCw, Lock, LogOut, LayoutDashboard, 
  ShoppingBag, Bell, Package, Plus, Edit2, Trash2, X, Ban, Search, Filter, Settings, Save
} from 'lucide-react';
// Added Sound for notification
// In a real app, import an asset. Here we use a base64 generic sound placeholder or just browser API.
// Since we can't load external assets easily without URL, we'll use a simple alert/toast logic for now visually
// or a simple base64 beep if requested, but visual is safer.
// Re-reading prompt: "audible and visual alert".

const NOTIFICATION_SOUND = 'data:audio/wav;base64,UklGRl9vT19XQVZFZm10IBAAAAABAAEAQB8AAEAfAAABAAgAZGF0YU'; // Short blip

export const AdminPanel: React.FC = () => {
  const [isAuthenticated, setIsAuthenticated] = useState(false);
  const [username, setUsername] = useState('');
  const [password, setPassword] = useState('');
  const [activeSection, setActiveSection] = useState<'ORDERS' | 'PRODUCTS' | 'SETTINGS'>('ORDERS');
  const [orders, setOrders] = useState<Order[]>([]);
  const [products, setProducts] = useState<Product[]>([]);
  const [config, setConfig] = useState<StoreConfig>({ address: '', phone: '' });
  ...
  const checkForNewOrders = () => {
    ...
    const sorted = currentOrders.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    
    if (sorted.length > 0) {
      const latest = sorted[0];
      // If we have a lastOrderId tracked, and the latest one is different (newer)
      // We rely on local state tracking. In a real app, this would be more robust.
      // For this demo, we check if the list length increased or ID changed.
      // Simple logic: store the latest ID in a ref or state.
  ...
  const refreshData = () => {
    ...
    const sorted = currentOrders.sort((a, b) => new Date(b.createdAt).getTime() - new Date(a.createdAt).getTime());
    setOrders(sorted);
    if (sorted.length > 0 && !lastOrderId) {
        setLastOrderId(sorted[0].id);
    }
  ...
  return (
    <div className="flex h-full bg-slate-50 overflow-hidden relative">
      {/* Toast Notification */}
      {newOrderAlert && (
        <div className="absolute top-4 left-4 right-4 md:left-8 md:right-auto md:w-80 bg-brand-blue text-white p-4 rounded-xl shadow-2xl z-50 animate-bounce-in flex items-center gap-4">
          <div className="bg-white/20 p-2 rounded-full">
            <Bell className="w-6 h-6 animate-pulse" />
          </div>
          <div>
            <h4 className="font-bold">سفارش جدید دریافت شد!</h4>
            <p className="text-sm text-blue-100">{newOrderAlert.customerName} - {newOrderAlert.totalAmount.toLocaleString()} تومان</p>
          </div>
          <button onClick={() => setNewOrderAlert(null)} className="mr-auto text-white/70 hover:text-white">
            <X className="w-5 h-5" />
          </button>
        </div>
      )}
      ...
      <aside className="w-64 bg-white border-l border-gray-200 hidden md:flex flex-col">
        ...
        <nav className="flex-1 p-4 space-y-2">
          ...
          <button
            ...
            className={`w-full flex items-center gap-3 px-4 py-3 rounded-xl transition-colors font-medium ${
              activeSection === 'SETTINGS' ? 'bg-brand-blue text-white shadow-md' : 'text-gray-600 hover:bg-gray-50'
            }`}
          >
      ...
      <main className="flex-1 overflow-y-auto p-4 md:p-8 pb-24">
        ...
        <div className="md:hidden flex gap-2 mb-6 overflow-x-auto pb-2 hide-scrollbar">
           <button
            onClick={() => setActiveSection('ORDERS')}
            className={`flex-1 min-w-[120px] flex items-center justify-center gap-2 px-4 py-3 rounded-xl border font-medium transition-all ${
          ...
          <button
            onClick={() => setActiveSection('SETTINGS')}
            className={`flex-1 min-w-[120px] flex items-center justify-center gap-2 px-4 py-3 rounded-xl border font-medium transition-all ${
              activeSection === 'SETTINGS' ? 'bg-brand-blue text-white border-brand-blue shadow-md' : 'bg-white border-gray-200 text-gray-600'
            }`}
          >
              ...
              <div className="bg-white p-4 rounded-xl shadow-sm border border-gray-100 flex flex-col md:flex-row gap-3">
                 <div className="flex-1 relative">
                    <input 
                      ...
                      className="w-full pl-4 pr-10 py-2.5 rounded-lg border border-gray-200 focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/10 outline-none"
                    />
                 ...
                 <div className="relative">
                    <select
                      ...
                      className="w-full md:w-48 pl-4 pr-10 py-2.5 rounded-lg border border-gray-200 focus:border-brand-blue focus:ring-2 focus:ring-brand-blue/10 outline-none appearance-none bg-white cursor-pointer"
                    >
        ...
        {activeSection === 'SETTINGS' && (
          <div className="space-y-6 animate-fade-in max-w-2xl mx-auto">
            ...
            <form onSubmit={handleSaveConfig} className="bg-white rounded-2xl p-6 shadow-sm border border-gray-100">
              <div className="space-y-5">
                <div>
                   <h3 className="font-bold text-gray-800 mb-4 border-b pb-2">اطلاعات تماس</h3>
                   <div className="space-y-4">
                     <div>
                       <label className="block text-sm font-bold text-gray-700 mb-2">شماره تماس فروشگاه</label>
                       <div className="relative">
                         <input
                            type="text"
                            value={config.phone}
                            onChange={(e) => setConfig({...config, phone: e.target.value})}
                            className="w-full pl-10 pr-4 py-3 rounded-xl border-2 border-gray-100 focus:border-brand-blue outline-none transition-all text-left ltr"
                     ...
                     <div>
                       <label className="block text-sm font-bold text-gray-700 mb-2">آدرس فروشگاه</label>
                       <div className="relative">
                         <textarea
                            rows={3}
                            value={config.address}
                            onChange={(e) => setConfig({...config, address: e.target.value})}
                            className="w-full pl-10 pr-4 py-3 rounded-xl border-2 border-gray-100 focus:border-brand-blue outline-none transition-all"
                ...
                <div className="pt-4">
                  <button 
                    type="submit"
                    className="w-full bg-brand-blue hover:bg-blue-900 text-white py-3 rounded-xl font-bold transition-colors shadow-lg flex items-center justify-center gap-2"
                  >
