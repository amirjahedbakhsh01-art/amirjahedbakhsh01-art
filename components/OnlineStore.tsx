...
import { getProducts, createOrder, getOrderById, updateOrderStatus } from '../services/dbService';
import { ShoppingCart, Plus, Minus, Trash2, CheckCircle2, ShoppingBag, UtensilsCrossed, Coffee, Milk, Beef, Snowflake, Sparkles, IceCream, Wallet, CreditCard, Search, X, PackageCheck, Cigarette, Ban } from 'lucide-react';
...
interface OnlineStoreProps {
  ...
  clearCart: () => void;
  onFly?: (rect: DOMRect, img: string) => void;
}
  ...
  const handleCancelOrder = () => {
    if (trackedOrder && trackedOrder.status === 'PENDING') {
       if (window.confirm('آیا از لغو سفارش اطمینان دارید؟')) {
          updateOrderStatus(trackedOrder.id, 'CANCELLED');
          // Refresh tracked order
          const updated = getOrderById(trackedOrder.id);
          setTrackedOrder(updated || null);
       }
    }
  };
                ...
                <div className="bg-gray-50 rounded-xl p-4 border border-gray-100">
                  ...
                  {trackedOrder.status === 'PENDING' && (
                    <button 
                      ...
                      className="w-full py-2 bg-red-50 text-red-600 rounded-lg text-sm font-bold border border-red-100 hover:bg-red-100 transition-colors flex items-center justify-center gap-2"
                    >
